...
@section('content')
<div class="h-full p-5 flex flex-col items-center gap-5">
    <div>
        <h2 class="text-5xl font-bold">Créez votre compte !</h2>
        <p>Inscrivez-vous pour commencer gérer vos taches.</p>
    </div>
   

    <div>
        <form action="{{ route('register') }}" method="post" class="w-full md:w-[350px] my-5 flex flex-col gap-3">
            @csrf
            ...
            <div class="py-7 text-center">
                <button class="w-full bg-blue-800 rounded-md p-2 text-white">@yield('title')</button>
                <p>Vous avez déjà un compte? <a href="{{ route('loginPage') }}" class="text-blue-800 underline">Connectez-vous</a></p>
            </div>
        </form>
        <a href="{{ route('google.redirect') }}" class="w-full md:w-[350px] p-2 rounded-md border-2 border-blue-80 flex flex-row items-center justify-center gap-5" >
